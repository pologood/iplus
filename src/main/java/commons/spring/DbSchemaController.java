...
public class DbSchemaController {
  ...
  public Map<String, Object> getDbSchema() {
    ...
    for (DbInfo info : dbs) {
      SingleConnectionDataSource c = new SingleConnectionDataSource(info.url, info.username, info.password, false);
      try {
  ...
  public String diffDbSchema(@RequestBody String body) {
    Map<String, Map<String, TableSchema>> expectMap = JsonHelper.readValue(body,
        new TypeReference<Map<String, Map<String, TableSchema>>>() {});
    ...
    for (Map.Entry<String, Map<String, TableSchema>> entry : expectMap.entrySet()) {
      ...
      Map<String, TableSchema> actualDbMap = (Map<String, TableSchema>) actualMap.get(entry.getKey());
